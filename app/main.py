...
async def handler(request: Request, exc: Exception):
    return JSONResponse(status_code=400, content={"error": str(exc), "hint": "Check payload keys and types."})
...
def predict(req: PredictRequest):
    ...
    return PredictResponse(prediction=yhat, model_version=VERSION, risk_flag=risk_flag, risk_threshold=risk_threshold)
